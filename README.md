...
> [!CAUTION]
> Personal Access Token (PAT) は機密情報です。`read:packages` スコープを持つトークンを生成し、`.npmrc` ファイルのパーミッションを適切に設定してください（例: `chmod 600 ~/.npmrc`）。このファイルを Git リポジトリにコミットしないよう注意してください。
...
- **GitHub Packages への公開設定**: GitHub Actions による CI/CD を整備し、パッケージビルドと GitHub Packages への公開を自動化しました。`opencode.jsonc` に `@yohi/opencode-geminicli-a2a-provider` を指定して他環境から直接インストール可能です。
- **バンドル依存の段階的解除** (進行中): 現在の「すべてを1つのファイルにバンドルする」ビルド設定を見直し、標準的な npm 依存関係の形へ移行してプラグインを軽量化する予定です。
