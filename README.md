...
- `LITELLM_PROXY_URL`: LiteLLM プロキシサーバーの URL (例: `http://localhost:4000`)
...
VS Code や DevPod を使用している場合、専用の Devcontainer 環境を構築済みです。

1. プロジェクトを VS Code で開き、`Reopen in Container` を選択します。
2. コンテナ起動時に、サイドカーとして **LiteLLM プロキシ** が自動的に立ち上がります。
3. `LITELLM_PROXY_URL=http://litellm:4000` を通じて、複数の A2A サーバーへのルーティングテストなどが即座に実施可能です。
