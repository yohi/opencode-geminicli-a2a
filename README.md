...
- **マルチモーダルサポート**: 画像やファイルの提供に対応し、A2A の仕様に基づくネイティブなマルチモーダル推論が 可能です。
- **動的ルーティングと耐障害性**: 動的なモデルレジストリ、クォータ枯渇などのエラー発生時における別モデルへの自 動フォールバック機構、および使用モデルに応じて別々の A2A サーバーへディスパッチするマルチエージェントルーティングをサポートしています。
- **推論パラメータの動的指定**: `generationConfig` (`temperature`, `topP`, `maxOutputTokens`, `stopSequences`  等) をサポートし、リクエスト単位でモデルの挙動を微調整可能です。
- **動的モデル切り替えとバックグラウンドタスク**: `DynamicModelRouter` を用いたタスクの複雑度に応じたモデル切り替え、コンテキスト維持、およびエフェメラルワークツリーでのバックグラウンドタスク（ヘッドレスワーカー）実行機能。
...
### 4. LiteLLM プロキシによる高度な構成 (任意)

本プロバイダーは [LiteLLM](https://github.com/BerriAI/litellm) プロキシサーバー経由での通信をネイティブにサポートしています。LiteLLM を使用することで、複数の A2A サーバーや異なる AI モデルを一元管理し、共通の API キーによる認証レイヤーを追加できます。

#### LiteLLM を使用するメリット
- **一元的なエンドポイント**: OpenCode 側からは LiteLLM の URL 1 つを参照するだけで、モデル名に応じて適切な A2A サーバーへ自動ルーティングされます。
- **セキュリティ**: A2A サーバーが直接インターネットに露出するのを防ぎ、LiteLLM の `Bearer Token` 認証を介してアクセスを制御できます。
- **ログと監視**: すべてのリクエスト・レスポンスを LiteLLM 側でロギングし、コストや使用状況を監視可能です。

#### 設定例 (`opencode.jsonc`)

OpenCode の設定ファイル（通常 `~/.config/opencode/opencode.jsonc` または `~/.opencode.jsonc`）において、プロバイダーとモデルを以下のように定義します。
...
```jsonc
{
  ...
  "models": [
    {
      "id": "gemini-3-flash", // この ID が LiteLLM へのリクエスト時の model 名になります
      "provider": "opencode-geminicli-a2a",
      "name": "Gemini CLI (A2A via LiteLLM) - Flash"
    },
    {
      "id": "gemini-3.1-pro", // LiteLLM 側の model_name と一致させてください
      "provider": "opencode-geminicli-a2a",
      "name": "Gemini CLI (A2A via LiteLLM) - Pro"
    }
  ]
}
```

#### LiteLLM 側の構成 (`litellm-config.yaml`)

LiteLLM の設定ファイルで、モデル名ごとに A2A サーバーの各ポートへ振り分ける設定を記述します。
...
model_list:
  - model_name: gemini-3-flash
    litellm_params:
      model: openai/a2a
      api_base: http://localhost:41242/
      api_key: "optional-a2a-token"
  - model_name: gemini-3.1-pro
    litellm_params:
      model: openai/a2a
      api_base: http://localhost:41243/
      api_key: "optional-a2a-token"

litellm_settings:
  drop_params: true # A2Aが理解できない不要なパラメータを削除
```

#### Devcontainer での利用

本リポジトリの `.devcontainer` 環境を使用している場合、コンテナ起動時に **LiteLLM プロキシが自動的にサイドカーとして立ち上がります**。

1.  コンテナ内で `LITELLM_PROXY_URL=http://litellm:4000` が自動設定されます。
2.  `litellm-config.yaml` がコンテナ内の LiteLLM に読み込まれ、即座にプロキシ経由のテストが可能です。
3.  ホスト側の A2A サーバーへ接続する場合は、`api_base: http://host.docker.internal:41242/` のように指定してください。
...
- 複数エージェントルーティング機能
- 動的モデル切り替えと非同期ワーカーディスパッチ（エフェメラルワークツリー対応）
