...
```jsonc
{
  ...
  // 2. デフォルトのプロバイダーとして指定する場合は定義を追記
  "provider": "opencode-geminicli-a2a",

  // 3. A2A サーバーとの接続設定（デフォルト値で機能する場合は省略可能）
  "a2aProvider": {
    "host": "127.0.0.1",
    "port": 41242,
    "protocol": "http"
...
>
   > ```powershell
   > # API キーを使用する場合
   > $env:GEMINI_API_KEY="your_api_key"; $env:CODER_AGENT_PORT="41242"; node "$env:APPDATA\npm\node_modules\@google\gemini-cli-a2a-server\dist\a2a-server.mjs"
   >
   > # 内部環境 (CCPA) の場合
   > $env:USE_CCPA="true"; $env:CODER_AGENT_PORT="41242"; node "$env:APPDATA\npm\node_modules\@google\gemini-cli-a2a-server\dist\a2a-server.mjs"
   > ```
   >
   > **プラットフォーム非依存な実行例 (npx を使用)**
   > npx 環境でパス解決を委譲する場合。互換性に有利です。
>
   > ```bash
   > npx cross-env GEMINI_API_KEY=your_api_key CODER_AGENT_PORT=41242 gemini-cli-a2a-server
   > ```

### 4. 環境変数による設定 (任意)

接続情報は `opencode.jsonc` 内の `a2aProvider` ブロック以外にも、以下の環境変数を利用して上書きすることが可能です（※ コード上の設定が優先されます）。
...
- **設定値の上書き（オーバーライド）の優先順位**:
  ...
  `geminiA2A(modelId, settings)` (前述の例で `createGeminiA2AProvider()` が返す関数) > `opencode.jsonc` の `a2aProvider` 指定等 (`createGeminiA2AProvider(options)` に渡る値) > 環境変数 (`GEMINI_A2A_*`)
